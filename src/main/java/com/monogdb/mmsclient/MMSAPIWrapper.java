...
public class MMSAPIWrapper {
	...
	public MMSAPIWrapper(String mms_username, String mms_api_key, String mms_server_address, String mms_server_port) {
		...
		restClient = ClientBuilder.newClient(new ClientConfig().register(JsonProcessingFeature.class).property(
	...
	private JsonObject get(String restURLStr) {
		int retryCount = 0;
		...
		while (retryCount++ <= maxRetryCount) {
			try {
				...
				if (retryCount <= maxRetryCount) {
					System.out.println("Error during MMS API call. Retry #" + retryCount + " of " +  maxRetryCount);
				} else {
					e.printStackTrace();
				}
			}
	...
	public JsonObject getHostList(String group) {
		String restURLStr = String.format("http://%s:%s/%s/groups/%s/hosts", mms_server_address, mms_server_port,
				MMS_API_URI, group);
		
		return get(restURLStr);
	}

	public JsonObject getHostMetricList(String group, String host) {
		String restURLStr = String.format("http://%s:%s/%s/groups/%s/hosts/%s/metrics", mms_server_address,
				mms_server_port, MMS_API_URI, group, host);
	...
	public JsonObject getHostMetric(String group, String host, String metric, String granularity, String period) {
		String restURLStr = String.format("http://%s:%s/%s/groups/%s/hosts/%s/metrics/%s?granularity=%s&period=%s",
				mms_server_address, mms_server_port, MMS_API_URI, group, host, metric, granularity, period);
